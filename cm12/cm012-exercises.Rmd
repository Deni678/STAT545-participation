...
gapminder %>% 
  ...
  ggplot() + geom_boxplot(aes(x = life_level, y = gdpPercap)) +
  labs(y = "GDP per capita, $", x= "Life expectancy level, years") + scale_x_discrete(drop=FALSE)
  theme_bw() 
```
...
- what class is  `continent`(a factor or character)? factor
...
gapminder1<-gapminder %>%
  filter(country %in% c("Canada","Germany","Australia", "Austria", "France"))
# Preserves information on levels
nlevels(gapminder1$country)
