...
ggplot(mpg, aes(cty, hwy)) +
  geom_jitter(alpha=0.5, size= 1) + geom_smooth(method="lm")+ theme_bw()
```
...
```{r, fig.width = 5, fig.height = 2}
gapminder %>% 
  # grouping inside of ggplot
  ggplot(aes(year, lifeExp, group=country, colour= country== "Rwanda")) +
  geom_line(alpha=0.5) + scale_colour_discrete("",labels=c("Other","Rwanda"))
```
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  scale_x_log10(labels=scales::comma_format()) + facet_wrap(~continent, scales="free_y")
```
...
gapminder %>% 
  ...
  scale_x_log10(labels = scales::comma_format()) + scale_size_area()
```
