...
gapminder %>% 
  ...
  summarize(min_life = min(lifeExp), country[lifeExp==min_life]) %>%
  arrange(min_life)
```
...
```{r}
#group by country and then sort by year (from lowest to highest year)
# mutate creates a new column (relative growth- compared to the first year population)
gapminder %>% 
  group_by(country) %>% 
  arrange(year) %>% 
